    ...
    while True:
        ...
        try:
            async with AsyncSessionLocal() as session:
                ...
                logger.info(f"📊 История цен: {len(cards)}")
    ...
    while True:
        ...
        try:
            async with AsyncSessionLocal() as session:
                ...
                await session.commit()
                logger.info(f"⏰ Затухание: {len(cards)}")
        except Exception as e:
...
async def loan_check_task():
    ...
    while True:
        ...
        try:
            async with AsyncSessionLocal() as session:
                now = datetime.utcnow()
                users = (await session.execute(select(User).where(User.loan_amount > 0, User.loan_due_at < now))).scalars().all()
                for u in users:
                    cards = (await session.execute(select(UserCard).where(UserCard.user_id == u.id).limit(3))).scalars().all()
                    for c in cards:
                    ...
                    u.trust_score -= 5
                    try:
                        await bot.send_message(u.telegram_id, "⚠️ <b>ПРОСРОЧКА!</b>\n\nБанк забрал твои карты за долг.", parse_mode="HTML")
                    except Exception:
                        pass
                await session.commit()
                logger.info(f"🏦 Просрочка: {len(users)}")
        except Exception as e:
            logger.error(f"Ошибка кредитов: {e}")
