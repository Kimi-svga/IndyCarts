...
async def decay_task():
    """Каждый час плавно возвращает цены к базовым."""
...
async def lifespan(app: FastAPI):
    logger.info("🚀 Запуск Indy Carts v0.5...")
    await init_db()
...
app = FastAPI(title="Indy Carts", version="0.5", lifespan=lifespan)
...
async def health():
    try:
        ...
        me = await bot.get_me()
        return {"status": "ok", "version": "0.5", "bot": me.username, "webhook": info.url}
    except Exception as e:
