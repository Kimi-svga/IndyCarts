...
async def cb_profile(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not user:
            ...
            return
        stmt = select(func.count(UserCard.id)).where(UserCard.user_id == user.id)
        count = (await session.execute(stmt)).scalar() or 0
        stmt = select(func.sum(Card.current_price)).join(UserCard, UserCard.card_id == Card.id).where(UserCard.user_id == user.id)
